...
test('CORS preflight OK on auth-refresh-provider-token (regression for BUG-012)', async ({ request }) => {
...
test('Service-role key is not in client bundle (regression for SEC-002)', async ({ page, request }) => {
  await page.goto('/sign-in');
  ...
  const scripts = await page.locator('script[src]').evaluateAll(els =>
    (els as HTMLScriptElement[]).map(s => s.src)
  );
