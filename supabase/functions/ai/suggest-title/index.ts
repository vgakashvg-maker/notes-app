// Edge Function: POST /functions/v1/ai/suggest-title
...
Deno.serve(async (req: Request) => {
  ...
  const prompt = parsePrompt(await readPrompt("suggest-title"));
  ...
  await logUsage(jwt, { task: "suggest_title", model: "llama3.2:3b", promptTokens: 0, completionTokens: 0, latencyMs: Date.now() - start });
  return new Response(JSON.stringify({ ok: true, data: { title } }), {
    status: 200,
